module RequestLogAnalyzer::FileFormat
  ...
  class SceaRails < Base

    extend CommonRegularExpressions
    
    SCEA_PREPEND = /\s*?\w+ \[.*?\] \d+ \d+(?:\s|\s<(.*)>\s)/
    ...
    def self.report_trackers(lines)
      ...
      analyze.frequency :player, :title => 'Player'
    ...
    RAILS_22_COMPLETED = /#{SCEA_PREPEND}Completed in (\d+)ms \((?:View: (\d+), )?DB: (\d+)\) \| (\d\d\d).+\[(http.+)\]/
    ...
    LINE_DEFINITIONS = {
      :processing => RequestLogAnalyzer::LineDefinition.new(:processing, :header => true,
            :teaser   => /Processing /,
            :regexp   => /#{SCEA_PREPEND}Processing ((?:\w+::)*\w+)#(\w+)(?: to (\w+))? \(for (#{ip_address}) at (#{timestamp('%Y-%m-%d %H:%M:%S')})\) \[([A-Z]+)\]/,
            :captures => [{ :name => :processed_player,     :type  => :string},
