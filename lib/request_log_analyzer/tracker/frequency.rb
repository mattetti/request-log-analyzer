  ...
  class Frequency < Base
    ...
    def report(output)
      ...
      if @categories.empty?
        ...
        table_opts = options[:title] ? {:id => options[:title].downcase.gsub(/\s/, '-'), :class => 'sortable'} : {:class => 'sortable'}
        output.table([{:align => :left, :title => 'Request'}, {:align => :right, :title => 'Hits' }, {:align => :right, :title => '%'}, {:type => :ratio, :width => :rest, :class => 'unsortable'}], table_opts) do |rows|
